...
def plot (sampless, samplers):
    for filename in filenames:
        ...
        real_mu_s_s[filename] = last_mu_s * np.std(wavelengths_window) + np.mean(wavelengths_window)
        real_sigma_s_s[filename] = last_sigma_s * np.std(wavelengths_window) 

    return real_mu_s_s, real_sigma_s_s
    
real_mu_s_s, real_sigma_s_s = plot(sampless, samplers)

for filename in filenames:
    c = 3.0E5
    velocity = (np.mean(real_mu_s_s[filename])-true_wavelength)/true_wavelength * c
    out = open('/Results_' + filename[:filename.index('.txt')] + '.txt', 'w')
